function [lat_mosaic, lon_mosaic, cdat_mosaic] = mosaic_plot(gopro_data)
  %Get the inputs fields
  if iscell(gopro_data)
    gopro_data = gopro_data{1};
  end
  gofields = fieldnames(gopro_data);
  gopro_elevs = gopro_data.elev;
  gopro_h_agls = gopro_data.h_agl;
  gopro_headings = gopro_data.heading;
  gopro_rolls = gopro_data.roll;
  gopro_pitchs = gopro_data.pitch;
  gopro_lats = gopro_data.lat;
  gopro_lons = gopro_data.lon;
  ...
  if ischar(gopro_paths) || length(gopro_paths)~=length(gopro_lats);
    ...
    gopro_paths = reshape(gopro_cells(strcmp(pathfn,gofields),:,:),size(gopro_data));
  end
