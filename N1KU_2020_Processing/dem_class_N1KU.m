...
classdef dem_class_N1KU < handle
  properties
  ...
  methods
    %% constructor
    function obj = dem_class_N1KU(param,res)
      ...
      % USA
      % -------------------------------------------------------------------
      % https://prd-tnm.s3.amazonaws.com/StagedProducts/Elevation/13/TIFF/n44w104/USGS_13_n44w104.tif 
      try
        new_dem_info.url = 'https://prd-tnm.s3.amazonaws.com/StagedProducts/Elevation/13/TIFF/n44w104/USGS_13_n44w104.tif';
        new_dem_info.res = [10 30];
        new_dem_info.res_str = {'10m' '30m'};
        new_dem_info.tile_en = [1 0];
        new_dem_info.subtile_en = [0 0];
        new_dem_info.tile_fn_ext = '_dem.tif';
        new_dem_info.mosaic_fn_fh = @(res_str) sprintf('%s/USGS_%s_dem.tif',res_str,res_str);
        new_dem_info.tile_fn_fh = @(x,y,res_str) sprintf('%s/%d_%d/%d_%d_%s.tar.gz',res_str,y,x,y,x,res_str);
        new_dem_info.acknowledge = '';
        new_dem_info.citation = '"USGS", https://www.usgs.gov/core-science-systems/ngp/3dep/data-tools';
        new_dem_info.y_tile_origin = 31;
        new_dem_info.x_tile_origin = 31;
        new_dem_info.y_tile_size = 100e3;
        new_dem_info.x_tile_size = 100e3;
        new_dem_info.no_data = -9999;
        new_dem_info.proj = artic_proj; % Loaded from proj_load_standard.m
        new_dem_info.out_path = ct_filename_gis(obj.param,fullfile('USA','DEM','USA'));
        obj.dem_info{end+1} = new_dem_info;
      catch ME
        warning(ME.getReport);
      end
