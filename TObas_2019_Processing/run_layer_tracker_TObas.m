...
params = ct_set_params(params,'cmd.generic',1,'day_seg','20190207_01');
% Specify specific frames (or leave empty/undefined to do all frames)
params = ct_set_params(params,'cmd.frms',[22]); 

% param_override.layer_tracker.debug_plots = {'debug'};

param_override.layer_tracker.echogram_img = 1; % To choose an image besides the base (0) image
...
switch ct_output_dir(params(1).radar_name)
  ...
  case 'accum'
    ...
    track_override.min_bin = 0.2e-6;
    track_override.threshold = 10;
    track_override.threshold_rel_max = -25;
    track_override.filter	= [5 3];
    track_override.filter_trim = [5 3];
    track_override.max_rng	= [0 3];
  ...
  case {'snow','kuband','kaband'}
    ...
    if 1
      track_override.init.max_diff_method = 'merge_vectors';
      track_override.init.method  = 'dem';
      track_override.init.dem_offset = 6.5e-9;
      track_override.init.dem_layer.name = 'surface';
      track_override.init.dem_layer.source = 'lidar';
      track_override.init.dem_layer.lidar_source = 'awi';
      track_override.init.max_diff = 0.4e-7;
    elseif 0
      track_override.init.method  = 'snake';
